public class RandomTree {
    ...
    */
    public init (data: [[String]], target: Int, perform: String, using: String, with: Int) {
   ...
   /*init creates original DataSet to be stored and creates tree with certain max depth
    Parameters:
        data -> data with labels
        target -> column number of label
        maxDepth -> maxDepth of each tree
        perform -> regression or classification
        using -> infoGain or giniIndex
        with -> num of random vars to consider at each iteration
    */
    public init (data : [[String]], target: Int, maxDepth: Int, perform: String, using: String, with: Int) {
        self.originalDataSet = DataSet(name: "Original", 
                                       data: data, 
                                       target: target
                                       )
...
public class DataSet {
    ...
    /*
      init for dataset to be used with the randomTree
            Parameters:
    ...
    */
    ...
    //methods returns the bestFeature i.e. max infoGain for the randomly chosen features
    public func getBestFeature(fromIndices: [Int]) -> Feature {
        var bestFeature = Feature(data: self.data, column: fromIndices.first!)
        var bestInfoGain : Float = bestFeature.getInfoGain(data: self.data, 
                                                           target: self.target)
    ...
    //methods returns the bestFeature i.e. min gini impurity for the randomly chosen features
    public func getGiniFeature(fromIndices: [Int]) -> Feature {
        var bestImpurity : Float 
        var bestFeature = Feature(data: self.data, column: fromIndices.first!)
       
        if (Float)(self.splitFeature.values.first!.name) == nil {
           bestImpurity = bestFeature.giniImpurityCategorical(data: self.data, 
                                                                    target : self.target) 
        } else {
           bestImpurity = bestFeature.giniImpurityNumerical(data: self.data, 
    ...
    //methods returns the bestFeature i.e. max stdDev reduction for the randomly chosen features
    public func getSplitFeature(fromIndices: [Int]) -> Feature {
        var bestFeature = Feature(data: self.data, column: fromIndices.first!)
        var bestSD : Float = bestFeature.getTargetStdDev(data: self.data, 
                                                               target: self.target)
