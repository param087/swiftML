...
public class SVC {
    ...
    //vars pertaining to the data 
      ...
      while true {
        ...
        for j in stride(from: 0, through: n-1, by: 1) {
          ...
          let Tk_ij = kernel(Tx_i, Tx_i) + kernel(Tx_j, Tx_j) * 2 * kernel(Tx_i, Tx_j)
          ...
          let bounds = computeLH(C : self.C, a_prime_i: a_prime_i, a_prime_j: a_prime_j, y_i: y_i, y_j: y_j)
          
          let L = bounds.0
          let H = bounds.1
          
          self.w = computeW(alphas: Tensor<Float>(alphas), x:self.X, y:self.Y)
 ...
 //predicts class of example   
  ...
  public func computeE(x_k : [Float], y_k : Float, w : Tensor<Float>, b : Float) -> Float {
  ...
  //computes weights
   public func computeW(alphas: Tensor<Float>, x: Tensor<Float>, y: Tensor<Float>) -> Tensor<Float>{
    
    let a = alphas.reshaped(to: [1, x.shape[0]])*y.reshaped(to: [1, x.shape[0]])
    return matmul(x.transposed(), a.reshaped(to: [8, 1])).reshaped(to: [1,  x.shape[1]])
  }
  
  //computes bias
  public func computeB(x: Tensor<Float>, y: Tensor<Float>,  w: Tensor<Float>) -> Float{
    
    let a = matmul(w.transposed().reshaped(to: [1,x.shape[1]]), x.transposed())
  ...
  public func computeLH(C : Float, a_prime_i: Float, a_prime_j: Float, y_i: Float, y_j: Float) -> (Float, Float){
    
    if y_i != y_j {
            return (max(0, a_prime_j - a_prime_i), min(C, C - a_prime_i + a_prime_j))
    }  else {
            return (max(0, a_prime_i + a_prime_j - C), min(C, a_prime_i + a_prime_j))
    }
    
  }
  
  
//two types of kernels defined: further kernels can be defined in the future or provided directly by the user

 public func linearKernel(x1 : Tensor<Float>, x2 : Tensor<Float>) -> Tensor<Float>{
    let d = x1.scalarCount
    return matmul(x1.reshaped(to: [1, d]), x2.reshaped(to: [d, 1]))
  }

  
   public func quadraticKernel(x1 : Tensor<Float>, x2 : Tensor<Float>) -> Tensor<Float>{
